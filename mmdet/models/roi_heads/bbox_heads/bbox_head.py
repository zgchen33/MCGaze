...
class BBoxHead(BaseModule):
    ...
    def __init__(self,
        ...
        if self.with_cls:
            ...
            self.fc_cls = build_linear_layer(
                ...
                out_features=cls_channels)  # 这里维度对不上，应该是继承的子类后来又重新定义更新了父类的这一属性
        if self.with_reg:
    ...
    def refine_bboxes(self, rois, labels, bbox_preds, pos_is_gts, img_metas):
        ...
        Example:
            ...
            >>> print(bboxes_list)
        """
        img_ids = rois[:, 0].long().unique(sorted=True) # rois [b*t*num_proposals,5]其中5为img_id,bbox4维坐标,这一行旨在提取出img_id
        assert img_ids.numel() <= len(img_metas)

        bboxes_list = []
        for i in range(len(img_metas)): # 对帧进行遍历
            inds = torch.nonzero(
                rois[:, 0] == i, as_tuple=False).squeeze(dim=1)
            num_rois = inds.numel()

            bboxes_ = rois[inds, 1:]    # 把上一个stage预测的，这一帧的bbox拿出来 [num_proposal,4]
            label_ = labels[inds]   # 这个是传入的一个初始化全为0的东西 [100]
            bbox_pred_ = bbox_preds[inds]   # 把本stage预测，这一帧的bbox拿出来
            img_meta_ = img_metas[i]
            pos_is_gts_ = pos_is_gts[i] # 这个也是传入的一个初始化全为0的东西

            bboxes = self.regress_by_class(bboxes_, label_, bbox_pred_,
                                           img_meta_)   # 根据本阶段预测的delta得到更新的bbox [x1,y1,x2,y2]
            ...
            bboxes_list.append(bboxes[keep_inds.type(torch.bool)])  # 这几行貌似啥都没做
    ...
    def regress_by_class(self, rois, label, bbox_pred, img_meta):
        ...
        if rois.size(1) == 4:
            new_rois = self.bbox_coder.decode(
                rois, bbox_pred, max_shape=max_shape) # rois应该是上个阶段query的bbox，bbox_pred是这阶段更新的tgt预测的bbox bbox_pred预测的好像是delta
        else:
            bboxes = self.bbox_coder.decode(
