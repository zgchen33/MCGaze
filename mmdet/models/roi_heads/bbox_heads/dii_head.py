...
class DIIHead(BBoxHead):
    ...
    def __init__(self,
        ...
        #     self.fc_cls = nn.Linear(in_channels, self.num_classes)   # 这个其实继承了父类里定义过，不过这里又重新定义一遍，值会更新的
        # else:
        ...
        #     self.blink_fcs.append(build_activation_layer(dict(type='ReLU', inplace=True)))
    ...
    def loss(self,
        ...
        face_labels = torch.cat([torch.stack([t[0]]) for t in labels])
        eyes_labels = torch.cat([torch.stack([t[1]]) for t in labels])
        head_labels = torch.cat([torch.stack([t[2]]) for t in labels])

        face_label_weights = torch.cat([torch.stack([t[0]]) for t in label_weights])
        eyes_label_weights = torch.cat([torch.stack([t[1]]) for t in label_weights])
        head_label_weights = torch.cat([torch.stack([t[2]]) for t in label_weights])

        face_bbox_targets = torch.cat([torch.stack([t[0]]) for t in bbox_targets])
        eyes_bbox_targets = torch.cat([torch.stack([t[1]]) for t in bbox_targets])
        head_bbox_targets = torch.cat([torch.stack([t[2]]) for t in bbox_targets])

        face_bbox_weights = torch.cat([torch.stack([t[0]]) for t in bbox_weights])
        eyes_bbox_weights = torch.cat([torch.stack([t[1]]) for t in bbox_weights])
        head_bbox_weights = torch.cat([torch.stack([t[2]]) for t in bbox_weights])

        face_bbox_pred = torch.cat([torch.stack([t[0]]) for t in bbox_pred])
        eyes_bbox_pred = torch.cat([torch.stack([t[1]]) for t in bbox_pred])
        head_bbox_pred = torch.cat([torch.stack([t[2]]) for t in bbox_pred])

        face_cls_score = cls_score[:,0,:]
        eyes_cls_score = cls_score[:,1,:]
        head_cls_score = cls_score[:,2,:]

        losses['face'] = self.head_loss(face_cls_score, face_bbox_pred, face_labels, face_label_weights, face_bbox_targets, face_bbox_weights, imgs_whwh[:, 0, :])
        losses['eyes'] = self.head_loss(eyes_cls_score, eyes_bbox_pred, eyes_labels, eyes_label_weights, eyes_bbox_targets, eyes_bbox_weights, imgs_whwh[:, 1, :])
        losses['head'] = self.head_loss(head_cls_score, head_bbox_pred, head_labels, head_label_weights, head_bbox_targets, head_bbox_weights, imgs_whwh[:, 2, :])
