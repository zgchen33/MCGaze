...
class TeViT(QueryInst):
    ...
    def extract_feat(self, B, T, img):
        ...
        if hasattr(self.backbone, 'msg_tokens'):
            x = self.backbone(B, T, img) #MsgShifT提取特征，返回四个layer的不同分辨率的特征
        else:
            x = self.backbone(img)
        if self.with_neck:
    ...
    def forward_train(self,
        ...
        B, T = img.size()[:2]  # 输入的img为[b,clip_len,3,w,h]
        ...
        proposal_boxes, proposal_features, imgs_whwh = \
            self.rpn_head.forward_train(x, img_metas) # 这里的proposal_boxes和proposal_feature本身只有100个，也就是不包含时间维度，函数里先单纯复制，因为反正是初始值，下一个函数开始迭代更新
