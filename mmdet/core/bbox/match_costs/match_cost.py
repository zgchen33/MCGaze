...
class BBoxL1Cost:
    ...
    def __call__(self, bbox_pred, gt_bboxes):
        ...
        bbox_cost = torch.cdist(bbox_pred, gt_bboxes, p=1) # 求正则项的p-norm
...
class FocalLossCost:
    ...
    def _focal_loss_cost(self, cls_pred, gt_labels):
        ...
        neg_cost = -(1 - cls_pred + self.eps).log() * (
            1 - self.alpha) * cls_pred.pow(self.gamma)  # -0.75* p^r * log(1-p) # p越接近0损失越小
        pos_cost = -(cls_pred + self.eps).log() * self.alpha * (
            1 - cls_pred).pow(self.gamma)  # -0.25* (1-p)^2 * log(p) p越大损失越小

        cls_cost = pos_cost[:, gt_labels] - neg_cost[:, gt_labels]
        # 结论是，该类的p越大，该类的损失cost越小，不管有多少类，因为都是各自类内算的，p没有softmax归一化，而是各自类sigmoid，和detr的区别在于加了log，这个式子直接抄tevit的就差不多
        # 匈牙利匹配用的argmin，也就是cost越小越会被匹配,没问题
        # 没搞懂在这里为什么需要加neg_cost?
