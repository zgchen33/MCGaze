...
class LoadImageFromFile:
    ...
    def __call__(self, results):
        ...
        img = mmcv.imfrombytes(
            img_bytes, flag=self.color_type, channel_order=self.channel_order) # 目前是按bgr读的
        if self.to_float32:
...
class LoadAnnotations:
    ...
    def _load_gazes(self, results):
        ...
        Returns:
            dict: The dict contains loaded label annotations.
        """
        results['gaze_fields'].append('gt_gazes')
        results['gt_gazes'] = results['ann_info']['gazes'].copy()
        return results
    ...
    def __call__(self, results):
        ...
        if self.with_bbox:
            ...
            if results is None:
                return None
        if self.with_label: 
            results = self._load_labels(results) # 就是新设立一个key 'gt_labels',值来源于以前的‘ann_info'中的labels
        if self.with_gaze:
            results = self._load_gazes(results)
        if self.with_mask:
