...
class Resize:
    ...
    def __call__(self, results):
        ...
        self._resize_img(results) # 根据scale来resize img
...
@PIPELINES.register_module()
class RandomFlip:
    """Flip the image & bbox & mask as well as gaze direction.
    ...
    def gaze_flip(self, gazes, direction):
        """Flip gaze horizontally.
        ...
        assert gazes.shape[-1] % 3 == 0 or gazes.shape[-1] % 2 == 0
        flipped = gazes.copy()
        if gazes.shape[-1] % 3 == 0:
            if direction == 'horizontal':
                flipped[..., 0::3] = - gazes[..., 0::3]
            elif direction == 'vertical':
                flipped[..., 1::3] = - gazes[..., 1::3]
            else:
                raise ValueError(f"Invalid flipping direction '{direction}'")
        elif gazes.shape[-1] % 2 == 0:
            if direction == 'horizontal':
                flipped[..., 0::2] = - gazes[..., 0::2]
            elif direction == 'vertical':
                flipped[..., 1::2] = - gazes[..., 1::2]
            else:
                raise ValueError(f"Invalid flipping direction '{direction}'")
        return flipped
    ...
    def __call__(self, results):
        ...
        maps as well as gaze direction.
        ...
        if results['flip']:
            ...
            # flip gazes
            for key in results.get('gaze_fields', []):
                results[key] = self.gaze_flip(results[key],
                                                      results['flip_direction'])
            # flip masks
            for key in results.get('mask_fields', []):
...
class CenterCrop:
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            margin_h = max(img.shape[0] - crop_size[0], 0)
            margin_w = max(img.shape[1] - crop_size[1], 0)
            offset_h =  int(margin_h / 2 + 0.5)
            offset_w =  int(margin_w / 2 + 0.5)
            # offset_h = np.random.randint(0, margin_h + 1)
            # offset_w = np.random.randint(0, margin_w + 1)
            crop_y1, crop_y2 = offset_h, offset_h + crop_size[0]
            crop_x1, crop_x2 = offset_w, offset_w + crop_size[1]
        ...
        for key in results.get('bbox_fields', []):
            ...
            valid_inds = (bboxes[:, 2] > bboxes[:, 0]) & (
                bboxes[:, 3] > bboxes[:, 1])
            if bboxes.shape[0] != 0 and bboxes.shape[0] != 1:# 排除ignore(无bbox)情况和只有head_bbox的情况，而face和eyes是同时有，同时没有的
                for ind in valid_inds:
                    if not ind:
                        # 因为数据集存在极少数错误，极少数本应在正中间的人脸，被放置在角落了 
                        # 所以在中央裁剪后，会出现without face bbox or eyes bbox的情况
                        # 针对这种情况，直接塞全图给face bbox 和 eyes bbox，毕竟样本本来就是有问题的，所以无所谓
                        bboxes[0, :] = bboxes[2, :]
                        bboxes[1, :] = bboxes[2, :]
                        valid_inds[:] = True
                        break
    ...
    def _get_crop_size(self, image_size):
        ...
        if self.crop_type == 'absolute':
            ...
            crop_size = np.asarray(self.crop_size, dtype=np.float32)
            # 原本下面为np.random.rand(2),但这会导致crop_h和crop_w不一样，故用rand(1)
            crop_h, crop_w = crop_size + np.random.rand(1) * (1 - crop_size)
            return int(h * crop_h + 0.5), int(w * crop_w + 0.5)
